...
case 'setlimits',
    ...
    % when one of the axes is created with plotyy, the second plot created
    % by plotyy disappears unless you select the second plot again so we
    % have to select the axes in reverse starting from the second to last
    % axes since the above for loop would have ended with the last axes
    % selected
    for c = (cl-1):-1:1
        axes(hAxes(c));
    end
